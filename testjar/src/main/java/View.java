...
public class View {
	...
	private static JPanel JPnaleList = null;
	...
	private static JPanel JPnaleDetail = null;
	...
	private static JPanel JPnaleConsole = null;
		...
		MouseListener musicListener = new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				Thread t1 = new Thread() {public void run() {
						...
						JPnaleConsole.remove(btnNext);
						JPnaleConsole.remove(btnStop);
						JPnaleConsole.remove(btnLast);
						...
						JPnaleConsole.add(btnLast);
						JPnaleConsole.add(btnStop);
						JPnaleConsole.add(btnNext);
	...
	private void initMusicConsole() {
		// TODO Auto-generated method stub
		JPnaleConsole = Component.getPanel(1300, 100);
		JPnaleConsole.setBackground(java.awt.Color.gray);
		...
		JPnaleConsole.add(playingMusic);
		JPnaleConsole.add(btnLast);
		JPnaleConsole.add(btnStop);
		JPnaleConsole.add(btnNext);
	...
	private void initMusicListDetail() {
		...
		JPnaleDetail = Component.getPanel(918, 700);
		...
		upSheet.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				// TODO Auto-generated method stub
				SheetHelper.uploadSheet(sheet);
				upSheet.setText("上传成功");
			}
		});
		...
		deleteSheet.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				// TODO Auto-generated method stub
				SheetHelper.deleteSheet(sheet.getUuid());
				...
				sheet = (MusicSheet) mySheet.get(0);
		...
		JPnaleDetail.add(cover);
		JPnaleDetail.add(blank);
		JPnaleDetail.add(Box);
		Box.add(intro);
		Box.add(upButtons);
		JPnaleDetail.add(listtitle);
		JPnaleDetail.add(list);
		for (int i = 0; i < musics.size(); i++) {
			final Music music = (Music) musics.get(i);
			JLabel name = Component.getLable(240, 40, 16, music.getName() );
	...
	private void initMusicList() throws HttpException, IOException {
		...
		JPnaleList = Component.getPanel(300, 700);
	...
	public void init() throws HttpException, IOException {
		...
		jFrame.add(JPnaleList);
		jFrame.add(JPnaleDetail);
		jFrame.add(JPnaleConsole);
	...
	private void initOtherSheetlist(int page) {
		...
		if (!isMy) {
			JPnaleDetail.remove(upMusic);
			JPnaleDetail.remove(upSheet);
			JPnaleDetail.remove(deleteSheet);
		}
